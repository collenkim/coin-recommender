...
# BR26: 적중률 하한의 기본값. 문턱이 실측 능력보다 높으면 "표본에서 우연히 높게 나온 코인"만
# 통과시킨다 -- 기존 레짐 트랙의 45% 문턱(실측 36.3%)이 그랬고, 워크포워드에서 표시 45% vs
# 실제 38.9%로 확인됐다. BR32부터는 트랙마다 따로 지정한다.
MIN_HIT_RATE = 0.25
...
class TrackSpec:
    ...
    min_samples: int = 10
    # BR32: 적중률 하한은 **트랙별**로 둔다. 목표가 클수록 도달률이 낮아지므로 하나의 문턱을
    # 공유하면 목표가 큰 트랙에서 "문턱이 실측 능력보다 높은" 상태가 된다 -- BR26에서 기존
    # 레짐 트랙(45% 문턱 vs 실측 36.3%)에 대해 고쳤던 것과 같은 결함이다.
    # 실측 도달률: 단타 36.4% / 중기 27.7% / 장기 20.9%. 각 문턱은 그 아래여야 한다.
    min_hit_rate: float = MIN_HIT_RATE


# BR32(2026-08-19): 4트랙 -> 3트랙. 보유 8시간 / 24시간 / 48시간으로 정리하고 중간 트랙 하나를
# 없앴다. 목표와 보유는 사용자 지정, 손절은 실측으로 정했다.
#
# | 트랙 | 목표/손절 | 보유 | 도달률 | 기대수익 |
# |---|---|---|---|---|
# | 단타 | +2%/-2% | 8시간 | 36.4% | +0.13% |
# | 중기 | +5%/-4% | 24시간 | 27.6% | +0.29% |
# | 장기 | +10%/-10% | 48시간 | 20.7% | +0.81% |
#
# 중기 손절은 -3% -> -4%로 넓혔다(+0.25% -> +0.29%).
#
# BR34: 장기 손절을 -10% -> **-5%**로 좁혔다. 기대수익만 보면 -10%가 낫지만(+0.86% vs +0.79%,
# -0.07%p), 3배 레버리지 기준 청산 여유가 **3%p -> 18%p로 6배**가 된다:
#   -3% 15.6%/+0.61%(여유 24%p) / **-5% 18.8%/+0.79%(18%p)** / -7% 20.0%/+0.83%(12%p) /
#   -10% 20.7%/+0.86%(3%p)
# 사용자가 3배 레버리지를 전제하므로 -0.07%p를 내주고 청산 위험을 줄이는 쪽을 택했다.
...
TRACKS = (
    TrackSpec(DAY, "단타", 0.02, 0.02, 8, "4h", min_hit_rate=0.25),
    TrackSpec(MID, "중기", 0.05, 0.04, 24, "4h", min_hit_rate=0.20),
    TrackSpec(LONG, "장기", 0.10, 0.05, 48, "4h", require_above_cloud=True, min_hit_rate=0.15),
)
