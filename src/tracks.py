...
# 타임프레임별 수집 깊이(일). 지정하지 않으면 설정값(16년 상한)을 쓴다.
#
# **30분봉은 BR27에서 판정봉이 되면서 상한을 풀었다.** 3년으로 두면 4시간봉 진입(9년치)의
# 표본 중 6년어치가 판정 데이터 부족으로 통째로 버려진다 -- 실측에서 30종 중 5종의 30분봉이
# 4시간봉보다 얕아 표본이 불균등했다. 판정봉은 진입봉과 같은 깊이를 가져야 한다.
#
# 15분봉은 어느 트랙도 쓰지 않으므로(진입 4h · 판정 30m) 2년만 받는다 -- 9년치는 316페이지에
# 20종 3GB인데 쓰는 곳이 없다. 사용자 요청("전 타임프레임 수집")을 지키되 깊이만 줄인 것이다.
