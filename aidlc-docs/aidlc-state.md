...
- [x] **(2026-08-19) BR28 거래소 최초봉 캐시 + 후보 30종** — 반복 요청 424→214/시간(실측 405→210요청, 37→18초). 후보 30종 전부 상장일까지 확보 완료. DB 779MB. 224/224 통과
- [x] **(2026-08-19) BR29 새 봉 없으면 조회 생략** — 요청 210→13(재실행 기준), 정상 운영 약 99/시간. 누적 424→99. 15분봉 미사용 사유는 "효과 없음"이 아니라 "이력 상한 2년으로 표본 35% 부족"임을 문서에 정정. 226/226 통과
- [x] **(2026-08-19) BR30 15분봉 9년 확보 + 데이터 구멍 수정** — 15분봉 2년 상한 해제, `_MAX_PAGES` 300→600. **증분이 1회 1,000봉 상한이라 15분봉 4년 격차를 못 따라잡고 구멍이 남던 결함 수정**(BNB 15m 2023~2026년이 통째로 비어 있었음, +148,583봉 복구). 전 타임프레임 충족률 99~100%. 공정 비교 결과 판정봉 30분 유지(15분과 차이 없음). DB 1,072MB. 227/227 통과
- [x] **(2026-08-19) BR31 주기 30분 + 중복 억제** — 주기 단축 전 **같은 4시간봉 신호가 8회 반복 검출되는 구조**를 발견해 중복 억제 선행(검출 8회→알림 1회 검증). 주기를 줄여도 진입 검출은 빨라지지 않음(4시간봉 마감이 병목)을 사용자에게 명시. 230/230 통과
- [x] **(2026-08-19) BR32 4트랙 → 3트랙** — 단타 +2%/8h · 중기 +5%/-4%/24h · 장기 +10%/-10%/48h. 미지정 파라미터는 격자 탐색으로 결정. **적중률 문턱을 트랙별로 분리**(장기가 실측 20.9% < 문턱 25%로 5/29종만 통과하던 결함 수정 → 12/29종). 알림 헤더 "구름대 돌파" → "골든크로스" 정정. 232/232 통과
- [x] **(2026-08-19) BR33 신규 상장 제외** — 상장 180일 미만 후보 제외. 상위 30 중 7종이 표본 1~4건으로 추천 불가인데 슬롯만 차지하던 문제 해결 → AAVE/UNI/XLM/LTC/TAO 등 편입. 거래량 기준은 `quoteVolume`(USDT 거래대금), 후보는 USDT 페어 한정 확인. 236/236 통과
- [ ] **미결정(사용자)**: 장기 트랙 3배 레버리지 시 청산 여유 3%p (손절 -10% → -30%, 청산선 -33%). 실측 5.0%가 청산선 초과 눌림 경험
- [x] **(2026-08-19) BR34 장기 손절 -5%** — 3배 청산 여유 3%p→18%p(기대수익 -0.07%p 대가). **급등 배제**(±0.06%p, 방향 없음)와 **손익분기 스탑**(도달률 절반, 기대수익 음수 전환)은 측정 후 기각. 손절 집행은 사용자 몫이며 발표 확률이 표시 손절 전제임을 알림에 명시. 237/237 통과
- [x] **(2026-08-19) BR35 역프 알림** — BTC 역프 -2% 이하를 시장 이벤트로 알림(매수 신호 아님, 실측 통계 병기). **최초 측정의 환율 기준·기간 오류를 사용자 지적으로 발견·정정**(KRW-USDT 2.2년 → USD/KRW 8.9년, -2% 사건 0건 → 713건). 충분한 표본으로 재검증한 결과 역프는 기저보다 나쁨(24h 상승확률 36% vs 52%). 상태 전환 시에만 발송. 247/247 통과
- [x] **(2026-08-20) BR36 빈 섹션 제거 + 도달 알림 개선** — 추천 0건 트랙 섹션 제거, 감시 주기 5분→2분. **트랙별 중복 방지 결함 수정**(BR32 PK 확장 때 `mark_price_event`만 따라가지 못해 한 트랙 도달이 나머지를 침묵시키던 문제). 도달 알림 분리·배치는 BR22로 이미 구현돼 있어 확인만 함. 251/251 통과
- [x] **(2026-08-20) BR37 운영 실적 표시 + 중복 저장 차단** — 실제 발송 추천의 매도가 도달률을 알림에 표시(표본 30건 미만이면 "판단하기 이릅니다" 병기). 중복 저장을 3중으로 차단(저장 단계·유니크 인덱스·집계 단계). **BR36 이전 오염 데이터 3건을 1분봉 기준 재판정으로 복구**하고 중복 103행 삭제(120행 → 17행, 실적 불변). 매수가는 측정 결과 진입봉 종가 유지. 261/261 통과
- [x] **(2026-08-20) BR38 가격 알림 수정** — 트랙별 목표·손절 비율이 기존 상수(-2%)로 하드코딩돼 중기 손절(-4%)이 잘못 표기되던 결함 수정(가격은 맞고 표기만 틀렸음). 진입가는 트랙 무관 동일하므로 하나로 병합. 266/266 통과
